...
void cgi::set_cgi(const Location& location, const std::string& path)
{
    ...
    bzero(resolved_path, PATH_MAX);
    if (realpath(location.get_cgi(extension).c_str(), resolved_path) == NULL)
        throw (500);
    this->args[0] = strdup(resolved_path);
}
...
void    cgi::cgi_init(const one_server& server, const request& req, const std::string& input)
{
    ...
    this->env_m["HTTP_COOKIE"] = req.get_header("Cookie");
...
void    cgi::cgi_execute()
{
    ...
    else if (!this->pid)
    {
        ...
        chdir_status = chdir(std::string(this->args[1]).substr(0, std::string(this->args[1]).find_last_of("/")).c_str());
        if (chdir_status != 0)
            exit(1);
