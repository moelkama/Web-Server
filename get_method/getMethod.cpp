...
/*   Updated: 2024/02/23 17:16:29 by moelkama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_method(request &req, manyServer *server, int newsockfd, int idx, cl& client) {
    ...
    if (isDirectory(client.path) && (req.get_path().back() != '/' && strlen(req.get_path().c_str()) > 1))
    {
        std::string response = "HTTP/1.1 301 Moved Permanently\r\n";
        response += "Location: "+  req.get_path() + "/" +"\r\n\r\n";
        int dd = send(newsockfd, response.c_str(), response.size(), 0);
        client.take_more_time();
        if (dd == -1)
        {
    ...
    if (client.location->_return)
    {
        ...
        if (client.test3 == 0) {
            ...
            std::string buffer = "<html><head>\n<meta http-equiv=\"Content-Type\" content=\"text/html;charset=UTF-8\"><title>Directory Listing</title></head><body>\n<h1>Directory Listing</h1><ul>\n";
            ...
            while ((entry = readdir(dir)) != nullptr) {
                ...
                if (entry->d_type == DT_REG) {
                    ...
                    if (buffer.find(entry->d_name) == std::string::npos)
                        buffer = buffer + "<li>file :  <a href=\"" + filename + "\">" + filename + "</a></li> \n";
                    }
                     else if (entry->d_type == DT_DIR && std::string(entry->d_name) != "." && std::string(entry->d_name) != "..") {
                            buffer += "<li> dir : <a href=\"" + filename + "\">" + filename + "</a></li>\n";
